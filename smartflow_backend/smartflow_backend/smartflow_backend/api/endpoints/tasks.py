...
async def breakdown_task(
    ...
    """
    try:
        task_data = await ai_task_service.breakdown_task(
            breakdown_in.task_description,
            breakdown_in.user_preferences
        )
...
async def create_subtask(
    ...
    """
    # 检查父任务是否存在
    parent_task = db.query(Task).get(task_id)
    if not parent_task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="父任务不存在"
...
async def update_subtask(
    ...
    # 检查用户是否有权操作此子任务
    if subtask.parent_task.owner_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="无权更新此子任务"
        )
...
async def delete_subtask(
    ...
    logger.info(f"用户 {current_user.username} 删除了子任务 {subtask_id}")