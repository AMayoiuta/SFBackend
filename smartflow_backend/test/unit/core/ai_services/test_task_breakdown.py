...
# 导入生产代码
from smartflow_backend.core.ai_services.task_breakdown import (
...
def breakdown_service(monkeypatch):
    """创建测试用的任务分解服务实例"""
    # 使用 monkeypatch 模拟配置 - 修复路径
    monkeypatch.setattr('smartflow_backend.core.config.settings.AI_TASK_BREAKDOWN_URL', 'https://mock-api.com')
    monkeypatch.setattr('smartflow_backend.core.config.settings.AI_API_KEY', 'test-key')
    monkeypatch.setattr('smartflow_backend.core.config.settings.AI_API_TIMEOUT', 5)
